...
$moduleToLoad = Get-Module -Name $optionalModule.ModuleName -ListAvailable -Verbose:$false -ErrorAction SilentlyContinue | Where-Object { $_.Version -ge [version]$optionalModule.ModuleVersion } | Sort-Object -Property Version | Select-Object -Last 1
if ($moduleToLoad) {
    ...
    if (($PSVersionTable.PSEdition -like "Desktop") -or ($PSVersionTable.PSEdition -like "Core" -and $isWindows)) {
        if ((Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction SilentlyContinue).Version -ge [version]'10.0') {
            ...
            Write-Warning "It is recommended to install module $($optionalModule.ModuleName) (min. v$($optionalModule.ModuleVersion)) to enable the possibility to enable Windows 10 toast notification for PackageUpdateInfo."
