...
Copy-Item -Path "$($WorkingDirectory)\PackageUpdateInfo" -Destination $publishDir.FullName -Recurse -Force
...
[System.IO.File]::WriteAllText("$($publishDir.FullName)\PackageUpdateInfo\PackageUpdateInfo.psm1", ($text -join "`n`n"), [System.Text.Encoding]::UTF8)
Remove-Item -Path "$($publishDir.FullName)\PackageUpdateInfo\internal" -Recurse -Force
Remove-Item -Path "$($publishDir.FullName)\PackageUpdateInfo\functions" -Recurse -Force
#endregion Update the psm1 file & Cleanup

#region Updating the Module Version
if ($AutoVersion) {
    Write-Host "Updating module version numbers."
    try { [version]$remoteVersion = @(Find-PSFModule 'PackageUpdateInfo' -Repository $Repository -ErrorAction Stop | Sort-Object Version -Descending)[0].Version }
    catch {
        throw "Failed to access $($Repository) : $_"
    }
    if (-not $remoteVersion) {
        throw "Couldn't find PackageUpdateInfo on repository $($Repository) : $_"
    }
    $newBuildNumber = $remoteVersion.Build + 1
    [version]$localVersion = (Import-PSFPowerShellDataFile -Path "$($publishDir.FullName)\PackageUpdateInfo\PackageUpdateInfo.psd1").ModuleVersion
    Update-PSFModuleManifest -Path "$($publishDir.FullName)\PackageUpdateInfo\PackageUpdateInfo.psd1" -ModuleVersion "$($localVersion.Major).$($localVersion.Minor).$($newBuildNumber)"
}
...
if ($ExportFunctions) {
    ...
    $functionFiles = Get-ChildItem -Path "$($WorkingDirectory)\PackageUpdateInfo\functions" -Filter '*.ps1' -Recurse
    Update-PSFModuleManifest -Path "$($publishDir.FullName)\PackageUpdateInfo\PackageUpdateInfo.psd1" -FunctionsToExport ($functionFiles.BaseName | Sort-Object)
}
