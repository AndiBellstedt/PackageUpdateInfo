...
if ($LocalRepo) {
    Write-Host "Creating Nuget Package for module: 'PackageUpdateInfo' at '$(Get-Location)"
    Publish-PSFModule -Path "$($publishDir)\PackageUpdateInfo" -DestinationPath . -SkipDependenciesCheck
    return
}

# Publish to Gallery
Write-Host "Publishing the PackageUpdateInfo module to $($Repository)"
$param = @{
    Path                  = "$($publishDir)\PackageUpdateInfo"
